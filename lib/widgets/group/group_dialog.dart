...
class _GroupDialogState extends State<GroupDialog> {
  ...
  Future<void> _loadContacts(String userId) async {
    ...
    try {
      ...
      print("Contacts snapshot has ${snapshot.docs.length} docs");

      final futures = snapshot.docs.map((doc) async {
        try {
          final details = await _contactService.getContactDetails(doc);
          return Contact(
            details['id'],
            details['userId'],
            details['userName'],
          ...
          print("Failed to fetch contact details for ${doc.id}: $e");
          return null;
        }
      });
  ...
  void _applyFilter() {
    setState(() {
      ...
      !_selectedMemberIds.contains(entry.key) &&
          ...
          .map((entry) => Contact('-1', entry.key, entry.value, 'Not In Contacts'));
            ...
            Wrap(
              ...
              children: [
                _buildCustomChip(
                  ...
                  onToggleAdmin: _selectedMemberIds.contains(widget.currentUserId)
                ...
                ..._selectedMemberIds
                    ...
                    .map((id) {
                  final contact =
                  _allContacts.firstWhere((c) => c.userId == id);
                ...
                ..._selectedMemberIds
                  ...
                  final unknownMember =
                  Contact('-1', id, username, 'Not In Contacts');
                  return _buildCustomChip(
                    label: unknownMember.userName,
                    isAdmin: _selectedAdminIds.contains(id),
                    onToggleAdmin: () => _toggleAdmin(unknownMember),
